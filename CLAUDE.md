...
- **Technical practice, architecture decision, limitation, or risk** → document in `docs_src/docs/development/` as a Markdown file with the `Spec` prefix (e.g. `SpecBackupStrategy.md`), and add a row to `docs_src/docs/development/Specifications.md` (the sidebar is not updated per file)

### Documentation page rules

All documentation pages must follow the design guidelines in `docs_src/docs/development/Dev-Documentation.md`. Key rules:
