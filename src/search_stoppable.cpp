...
void SearchStoppable::saveSearchHistoryToTable(const QString &connectionName)
{
    // Save Search to db
    ...
    if (!query.exec()) {
        qWarning() << "SearchStoppable::saveSearchHistoryToTable - Query error:" << query.lastError().text();
    }
}

void SearchStoppable::loadSearchHistoryCriteria()
{
    // Query
    QSqlQuery query(QSqlDatabase::database(connectionName));
    QString querySQL = QLatin1String(R"(
    ...
    if (!query.exec()) {
        qWarning() << "SearchStoppable::loadSearchHistoryCriteria - Query error:" << query.lastError().text();
        return;
    }
