...
whose delivery is owned by `SpecK3Deployment.md` (`DPL-F5`) — and the
**light/dark variant of the fallback icon set** (`THM-F11`). *(Widened
2026-09-23, twice.)* Recorded as `[Backlog]` only, 3.1 candidates, not authorised
for 3.0: a Katalog colour-scheme setting (`THM-F12`, `THM-C15`) and a startup
workaround for a misread portal preference (`THM-F13`, `THM-C16`). *(Added
2026-09-24.)*
...
- **K2 (`qt_widgets`)** — in maintenance mode. It shares the same
  ...
  (`THM-C5`).
- **Icon theme** — icon set selection is not governed here, **except** that the fallback icon set follows light/dark (`THM-F11`); `THM-F7` sets a
...
| THM-F11 | K3's **fallback icon set** — the light set (`:/fallback-icons`) or the dark set (`:/fallback-icons-dark`) — follows the **same** light/dark preference as `DPL-F5` (`SpecK3Deployment.md`), in **every** K3 build, packaged or built from source: when Qt reports the preference (`Qt.styleHints.colorScheme` / `QStyleHints::colorScheme`) as **Dark**, the dark set is used; as **Light**, the light set; as **Unknown**, the existing test on the application palette's window lightness decides, so a Plasma source build keeps its current behaviour. When the preference changes while Katalog runs, the icon set follows **without a restart**, and icons already on screen are redrawn from the new set. Reported 2026-09-23: in dark mode the packaged build showed light-mode icons, because the set was chosen once at startup from the palette alone (`qt_quick/main.cpp`). Approved by the maintainer on 2026-09-23. This row authorises the `qt_quick` source change it needs (`main.cpp` and whatever QML/C++ the live refresh requires); `SpecK3Deployment.md`'s "no `qt_quick` source change" sentence governs that page's own rows only, as `THM-C14` already records. It adds, changes and removes **no** user-visible string and changes nothing under `core/`. | [Planned] |
| THM-F12 | **3.1 candidate.** The Settings page offers a choice of Katalog's colour scheme: **System** (follow the desktop — the default), **Light** or **Dark**. The choice applies **immediately, without restart**, and drives **one** effective scheme from which both the Kirigami / Fusion colours (`DPL-F5`) and the fallback icon set (`THM-F11`) are derived. **System** behaves exactly as `DPL-F5` and `THM-F11` do today. Recorded at the maintainer's request on 2026-09-24 as a candidate for **Katalog 3.1**, not for 3.0. It is a new feature with no K2 equivalent, so under the project rule that new UI features follow K2 parity it is the maintainer's explicit decision; the open points under *Open, not authorised* must be settled before it can move to `[Planned]`. | [Backlog] |
| THM-F13 | **3.1 candidate.** On Linux, when Qt reports the colour scheme as **Unknown** at startup, Katalog reads the preference directly from the XDG desktop portal with `Settings.ReadOne` (`org.freedesktop.appearance` / `color-scheme`), so a host whose portal misreports it through `ReadAll` — the Linux Mint / Cinnamon case in `SpecK3Deployment.md` *Known limitations* — starts in the correct scheme. Live changes continue to come from Qt. Recorded on 2026-09-24; the maintainer chose to ship 3.0 with the known limitation rather than add a dependency. | [Backlog] |
...
| THM-C15 | **3.1 candidate, with `THM-F12`.** The choice is stored in a **new** key (working name `Settings/ColorScheme`) in the same settings file. An **absent** key means **System**, so existing 3.0 beta settings files keep working unchanged and need no migration; K2 does not read the key and MUST NOT be changed for it (`THM-C5`). The effective scheme is **provided by Katalog** and read by `Theme.qml` and by the fallback icon selection **instead of** `Qt.styleHints.colorScheme` directly; implementing this therefore amends `DPL-C6` and `THM-F11`, which today read Qt's value. | [Backlog] |
| THM-C16 | **3.1 candidate, with `THM-F13`.** `THM-F13` requires linking **`Qt6::DBus`**, on Linux only. That is a new Qt module in `CMakeLists.txt` and a new library in every Linux package; per the project rule on new libraries it needs the maintainer's **explicit approval at implementation time**, and recording `THM-F13` here does not give it. | [Backlog] |
...
| `THM-F12` strings | Only **"System"** exists today (the K3 first-run dialog). **"Light"**, **"Dark"** and a label for the setting would be **new** user-visible strings, each needing per-string approval and a translation slot in all 30 languages. Not decided. |
| `THM-F12` on Plasma source builds | Source builds on Plasma use `org.kde.desktop` and KColorScheme, not the bundled `Theme.qml`. Forcing Light or Dark there needs `KColorSchemeManager` (the KF6 `KColorScheme` component, a new dependency needing approval), **or** the override is limited to packaged builds. Not decided. |
| `THM-F12` is new, not a portage | No K2 equivalent exists; the project rule is that new UI features come after K2 parity. Taking it on in 3.1 is the maintainer's explicit decision to make. |
...
- **THM-F11** — On each of: the packaged Linux build on a non-Plasma desktop, the Windows and macOS portable builds, and a Plasma source build — start Katalog in dark mode: icons drawn from the fallback set are the dark variants (light glyphs on the dark surface). Start in light mode: the light variants. With Katalog open, switch the system between light and dark: the icons already on screen change set without a restart. On the Plasma source build, confirm nothing changes against today's behaviour in either mode.
- **THM-F12 / THM-C15** *(Backlog, 3.1)* — With no `Settings/ColorScheme` key in an existing beta settings file, start Katalog: it behaves as **System**. Choose Light, then Dark, then System on a dark desktop: colours, controls and fallback icons switch each time without a restart. Open K2 on the same settings file: nothing changes.
- **THM-F13 / THM-C16** *(Backlog, 3.1)* — On the Linux Mint / Cinnamon tester VM, start the AppImage with the desktop dark: it opens dark. Confirm `Qt6::DBus` is linked on Linux only and that the maintainer approved it.
