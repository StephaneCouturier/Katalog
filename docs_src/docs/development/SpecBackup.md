...
| BKP-F22 | The K3 Backup page's per-link menu carries a **Duplicate** action that creates a new link copying every setting of the original: type (Backup/Archive), source device, target device, `strictCopy`, conflict mode, source mode (Catalog/Drive) and Include empty (`BKP-F17`). The new link's name is the original name followed by `_` and a datetime stamp in the `YYYYMMDD-HHmmss` shape of `BKP-F5` — no literal word is added, so the generated name carries no translatable text. The duplicate starts with **no run history**: last-run date and last-run bytes (`BKP-F10`) are empty, as for a newly created link. The action creates the link and nothing else — it never starts a run, a preview or a replication, and never modifies the original. The link list refreshes with the new link present. Link names are not required to be unique; two duplicates made in the same second share a name, which is accepted, not an error. | [Planned] |
| BKP-F23 | A link is created from the Backup page's create-link form: the user names it (a helper composes the name as `source → target` from the two chosen devices), picks a source and a target catalog device, the type (Backup/Archive) and the options (`strictCopy`, conflict mode, source mode, Include empty). Creation is refused, with a message and no link created, when the name is empty, when either device is not chosen, or when source and target are the same device. | [Implemented] |
| BKP-F24 | An **Edit** action reopens an existing link's settings and saves the changes back to that same link, under the same validation as creation (`BKP-F23`). Shipped in K3 only — see the divergence note below. | [Implemented] |
| BKP-F25 | An **Invert** action swaps a link's source and target devices. When the link's name contains an arrow (` → ` or the legacy ` -> `), the two halves of the name are swapped with it so the name keeps matching the direction; a name without an arrow is left unchanged. Inverting changes the mapping only and moves no file. | [Implemented] |
| BKP-F26 | The result of a preview (`BKP-F16`) can be exported to a CSV file written to the collection folder, and the path of the written file is reported to the user. | [Implemented] |
| BKP-F27 | A **Delete** action removes a link. Deleting removes the mapping only: it never touches any file on the source or the target device (`BKP-C1` / `BKP-C3`). In K3 the action is confirmed by a dialog naming the link before anything is removed — see the divergence note below. | [Implemented] |
| BKP-F28 | Every change to a link (create, edit, invert, duplicate, delete) is persisted to the collection's mapping file as well as to the database, so the link list survives closing and reopening the collection. | [Implemented] |

> **Open divergences recorded against `BKP-C6`, awaiting a ruling.** `BKP-F23`–`BKP-F28`
> document link management as it ships today, and writing them down exposes two
> places where K2 and K3 are **not** identical, which `BKP-C6` requires them to be.
> Neither is resolved here and neither is authorised by the `BKP-C6` exception:
> (a) **Edit** (`BKP-F24`) exists in K3 only — K2 offers no edit or rename path,
> only create; (b) **Delete** (`BKP-F27`) is confirmed by a dialog in K3 but is
> carried out on a single menu click in K2, with no confirmation. (b) is reported
> as a defect, not as an authorised difference: no requirement permits an
> unconfirmed destructive action. A third, lesser difference is placement only and
> is not a behaviour divergence: K2 exposes preview CSV export (`BKP-F26`) as a
> button on the preview area while K3 exposes it in the link menu, and K2 keeps Run
> and Preview in the link menu while K3 puts them on the link card. The user rules
> on (a) and (b); until then these rows describe reality and authorise nothing new.
...
| BKP-C6 | K2 and K3 have identical backup behaviour; both call the same `core/` code. User-visible strings are byte-identical in both: the `BKP-F17` control uses the field label `Directories` (an existing K2 string, reused — no new translation slot) and the checkbox text `Include empty` (one new string). **Exception, and only where a requirement names it explicitly:** `BKP-F18`–`BKP-F21` and `BKP-F22` are K3-only and have no K2 counterpart. What a single backup *run* does stays identical between the two — the divergence is confined to K3 offering to start several runs in sequence. No other divergence is authorised by this exception. | [Implemented] |
...
| BKP-C13 | `BKP-F22` adds **no core code**: the UI adapter reads the link with `BackupMappingManager::getMappingById()` and creates the copy with the existing `createMapping()`, then persists with `saveMappingTableToFile()` and signals the list to refresh — the same sequence link creation already uses. No new SQL enters the UI layer (`BKP-C7`). The datetime stamp is composed in the adapter. | [Planned] |
| BKP-C14 | `BKP-F22` spends **one** new translation slot: the menu label `Duplicate`. The generated link name contains no translated text by construction (`BKP-F22`), and the duplicate needs no dialog. Any second string requires its own per-string approval before it is written. | [Planned] |
...
- **BKP-F22** — Duplicate a link that has been run at least once. A new link appears named `<original>_YYYYMMDD-HHmmss`; open it and confirm every field matches the original, including Include empty and the conflict mode. Its last-run date and last-run size are empty. The original is unchanged and still shows its own run history.
- **BKP-F22 (no side-effect)** — Duplicate an Archive link. No run, preview or replication starts; nothing is copied or moved on either device.
- **BKP-F22 (persistence)** — Duplicate a link, close and reopen the collection. The duplicate is still there with all its settings.
- **BKP-F23** — Try to save a new link with an empty name, then with no target, then with the same device as source and target. Each is refused with a message and no link is created. Correct it and save: the link appears in the list.
- **BKP-F25** — Invert a link whose name holds an arrow. Source and target are swapped and the two halves of the name swap with them. Invert a link whose name has no arrow: the devices swap and the name is left as it was.
- **BKP-F26** — Run a preview, then export it. A CSV file is written to the collection folder and its path is reported.
- **BKP-F27** — Delete a link. The link disappears from the list and no file on the source or target device is removed.
- **BKP-F28** — Create, invert and delete links, then close and reopen the collection. The list matches what was left behind.
